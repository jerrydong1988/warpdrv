...
// Completed model downloads should become visible without a manual scan. Keep
// this fire-and-forget so a scanner failure cannot turn a successful download
// into a failed one. Dynamic imports avoid coupling route initialization to the
// download manager's module initialization.
function triggerModelRescan(): void {
	void Promise.allSettled([
		import('../routes/models').then(({ rescanAllModels }) => rescanAllModels()),
		import('../routes/whisperModels').then(({ refreshWhisperModelsCache }) => refreshWhisperModelsCache()),
	]).then(results => {
		for (const result of results) {
			if (result.status === 'rejected') {
				console.warn('[DownloadManager] model rescan failed:', result.reason);
			}
		}
	});
}
