...
function parseNewFormatLine(serverId: TServerId, line: string): void {
	...
	// New llama-server logs use n_gen; older builds used n_decoded.
	const genMatch = line.match(/slot\s+print_timing:\s*id\s+\d+\s*\|\s*task\s+\d+\s*\|\s*(?:n_gen|n_decoded)\s*=\s*(\d+),\s*tg\s*=\s*[\d.]+\s*t\/s/);
	if (genMatch) {
