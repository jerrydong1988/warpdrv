...
export class ChatSearchToolService {
	...
	async searchMessages(query: string, threadId: string, limit: number, page: number): Promise<IChatSearchResponse> {
		const offset = (page - 1) * limit;
		const allResults = await this.persistence.searchMessages(query, { mode: 'everywhere', limit, offset });
