    ...
    api.onReady(() => {
        ...
        api.eventNode.on('/warpcore', 'bridge.inference.finish', async (eventApi) => {
            ...
            const applicableGuardrails = activeGuardrails.filter(g => {
                ...
                // triggerOnTools is typed as IToolAttachment[]; string[] entries
                // are tolerated at runtime
                const triggerList = g.triggerOnTools as unknown as Array<string | { toolName?: string }>;
                const tools = typeof triggerList[0] === 'string'
                    ? (triggerList as string[]).join(',').split(',').map((t: string) => t.trim().toLowerCase()).filter(Boolean)
                    : triggerList.map((t) => (t as { toolName?: string }).toolName?.toLowerCase() ?? '');
