...
describe("auth routes", () => {
	...
	it("requires a cookie on loopback when localhost authentication is enabled", async () => {
		await store.put<ISettings>("settings:general", {
			...DEFAULT_SETTINGS,
			apiAuthEnabled: true,
		...
		try {
			...
			await store.put<ISettings>("settings:general", DEFAULT_SETTINGS);
		}
	});
});
