	...
	({
		...
		const perChatLabel =
			params.contextSize === 0
				? t("servers:contextKV.effectivePerChatAuto")
				: params.kvUnified
					? t("servers:contextKV.effectivePerChatUnified", { effective: fmtK(perChat) })
					: params.parallelSlots > 1
						? t("servers:contextKV.effectivePerChatSplit", {
								effective: fmtK(perChat),
								total: fmtK(params.contextSize),
								slots: params.parallelSlots,
							})
						: t("servers:contextKV.effectivePerChat", { effective: fmtK(perChat) });
		...
		return (
			<Card>
				<VStack align="stretch" gap="4">
					{hasModelContext ? (
						<SliderNumberField
							...
							suffix={
								params.contextSize === 0
									? t("common:ui.0Auto")
									: t("servers:checkpoints.maxContext", {
											count: (maxContext / 1024).toFixed(0),
										})
							}
							logarithmic
						/>
					...
					>
						{t("common:ui.kvCacheQuantization")}
					</Text>
					<Flex gap="4">
						<SelectField
							label={t("common:ui.kType")}
					...
					<NumberField
						label={t("common:ui.parallelSlots")}
