...
export const Footer = React.memo(({
	...
	return (
		<Flex px="6" py="4" justify="space-between" align="center" borderTopWidth="1px" borderColor="var(--wc-border-subtle)" bg="var(--wc-bg-surface)">
			<HStack gap="4">
<Switch.Root label={t('common:ui.autoLaunchAtStartup')} checked={autoLaunch} onCheckedChange={(d) => onAutoLaunchChange(d.checked)} color={autoLaunch ? 'var(--wc-accent-blue)' : 'var(--wc-text-tertiary)'}>
				<Switch.HiddenInput />
				<Switch.Control css={{ bg: autoLaunch ? 'var(--wc-accent-blue)' : 'var(--wc-bg-card)' }}>
					<Switch.Thumb css={{ bg: 'var(--wc-special-switch-thumb)' }} />
				</Switch.Control>
				<Switch.Label ml="2" fontSize="13px" color={autoLaunch ? 'var(--wc-accent-blue)' : 'var(--wc-text-tertiary)'} userSelect="none">{t('launch.autoLaunch')}</Switch.Label>
				</Switch.Root>
				<Switch.Root label={t('common:ui.autoLoadLatestCheckpointOnStart')} checked={autoLoadCheckpoint} onCheckedChange={(d) => onAutoLoadCheckpointChange(d.checked)} color={autoLoadCheckpoint ? 'var(--wc-accent-blue)' : 'var(--wc-text-tertiary)'}>
					<Switch.HiddenInput />
					<Switch.Control css={{ bg: autoLoadCheckpoint ? 'var(--wc-accent-blue)' : 'var(--wc-bg-card)' }}>
						<Switch.Thumb css={{ bg: 'var(--wc-special-switch-thumb)' }} />
					</Switch.Control>
					<Switch.Label ml="2" fontSize="13px" color={autoLoadCheckpoint ? 'var(--wc-accent-blue)' : 'var(--wc-text-tertiary)'} userSelect="none">{t('launch.autoLoadCheckpoint')}</Switch.Label>
				</Switch.Root>
				<Switch.Root label={t('common:ui.autoSaveCheckpointOnStop')} checked={autoSaveCheckpoint} onCheckedChange={(d) => onAutoSaveCheckpointChange(d.checked)} color={autoSaveCheckpoint ? 'var(--wc-accent-blue)' : 'var(--wc-text-tertiary)'}>
				<Switch.HiddenInput />
				<Switch.Control css={{ bg: autoSaveCheckpoint ? 'var(--wc-accent-blue)' : 'var(--wc-bg-card)' }}>
					<Switch.Thumb css={{ bg: 'var(--wc-special-switch-thumb)' }} />
				</Switch.Control>
				<Switch.Label ml="2" fontSize="13px" color={autoSaveCheckpoint ? 'var(--wc-accent-blue)' : 'var(--wc-text-tertiary)'} userSelect="none">{t('launch.autoSaveCheckpoint')}</Switch.Label>
				</Switch.Root>
			</HStack>
			<HStack gap="2">
<Button size="sm" variant="ghost" color="var(--wc-text-tertiary)" _hover={{ color: 'var(--wc-text-secondary)', bg: 'var(--wc-bg-hover)' }} borderRadius="lg" fontSize="13px" onClick={onCancel}>{t('actions.cancel')}</Button>
				{isEdit ? (
					<>
						<Button size="sm" disabled={!canLaunch || launching}
							...
							onClick={onSave}>{t('actions.save')}</Button>
						<Button size="sm" disabled={!canLaunch || launching}
