...
export const LaunchServerDialog = React.memo(({ onClose, serverId }: ILaunchServerDialogProps) => {
	...
	const handleSaveWithoutRelaunch = async () => {
		...
		setLaunching(false);
		if (result.ok) { toast('success', t('toast.serverConfigSaved')); onClose(); }
		else toast('error', result.error ?? t('toast.saveConfigFailed'));
	};
	...
	const handleLaunch = async () => {
		...
		if (server) {
			...
			setLaunching(false);
			if (result.ok) { toast('success', t('toast.serverRelaunched')); onClose(); }
			else toast('error', result.error ?? t('toast.relaunchFailed'));
		} else {
			...
			setLaunching(false);
			if (result.ok) { toast('success', t('toast.serverLaunchedOnPort', { port: result.data.port })); onClose(); }
			else toast('error', result.error ?? t('toast.launchFailed'));
		}
	};
			...
			>
				...
				<Flex px="6" py="4" justify="space-between" align="center" borderBottomWidth="1px" borderColor="var(--wc-border-subtle)" bg="var(--wc-bg-surface)">
					<HStack gap="3">
						...
						<Box>
<Text fontSize="16px" fontWeight="700" color="var(--wc-text-primary)" letterSpacing="-0.01em">{server ? t('launch.editServer') : t('launch.title')}</Text>
						<Text fontSize="12px" color="var(--wc-text-tertiary)">{server ? t('launch.editServerDesc') : t('launch.launchServerDesc')}</Text>
						</Box>
					</HStack>
				...
				<Box flex="1" overflowY="auto" p="6">
					<Flex gap="6">
						...
						<VStack align="stretch" gap="5" flex="1" minW="0">
							...
							<ServerInfoCard serverName={serverName} onServerNameChange={setServerName} port={params.port} onPortChange={v => updateParam('port', v)} aliases={serverAliasesInput} onAliasesChange={setServerAliasesInput} placeholder={selectedEntry?.file.fileName.replace('.gguf', '') ?? t('common:ui.leaveEmptyForAutoGeneratedName')} />
							<BackendPickerCard
						...
						<VStack gap="5" flex="1" minW="0" align="stretch">
							...
							<RecommendedParamsCard
								...
								onSave={async (modelId, text) => {
									...
									if (result.ok) toast('success', t('toast.recommendedParamsSaved'));
