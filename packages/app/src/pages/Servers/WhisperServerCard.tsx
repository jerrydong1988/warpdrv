...
export const WhisperServerCard = React.memo(({
	...
	const { mutate: stopMut } = useMutation<string, void>(useCallback(async (id: string): Promise<IApiResponse<void>> => {
		await stopWhisperServer(id);
		return { ok: true, data: undefined, error: null };
	}, []));
	const { mutate: restartMut } = useMutation<string, void>(useCallback(async (id: string): Promise<IApiResponse<void>> => {
	...
	const updateCallback = useCallback(async ([id, data]: [string, Partial<Pick<IWhisperServer, 'serverAlias'>>]): Promise<IApiResponse<IWhisperServer | null>> => {
		const server = await updateWhisperServer(id, data);
		return server !== null
			? { ok: true, data: server, error: null }
			: { ok: false, data: null, error: 'Update failed' };
	}, []);
	const { mutate: updateMut, loading } = useMutation<[string, Partial<Pick<IWhisperServer, 'serverAlias'>>], IWhisperServer | null>(updateCallback);
							...
							<HStack gap="3" alignItems="center" flexWrap="wrap">
								...
								<Popover.Root lazyMount unmountOnExit open={addingAliasOpen} onOpenChange={(details) => { if (!details.open) { setAddingAliasOpen(false); setNewAliasValue(''); } }}>
									<Popover.Trigger asChild>
										<Badge px="1.5" py="0.25" borderRadius="md" fontSize="11px" fontFamily='"Geist Mono", monospace' bg="var(--wc-special-indigo-bg-subtle)" color="var(--wc-special-indigo)" borderWidth="1px" borderColor="var(--wc-special-indigo-border-subtle)" cursor="pointer" onClick={(e) => { e.stopPropagation(); setAddingAliasOpen(true); }} title={t('common:ui.addAlias')}>
											<Plus size={10} />
										</Badge>
									</Popover.Trigger>
									<Portal>
