...
import {
	Box, Text, HStack, VStack, Flex, Button, Badge,
} from '@chakra-ui/react';
import {
	X, Download, Pause, Play, CheckCircle, AlertCircle,
	XCircle, Clock,
} from 'lucide-react';
				...
				{filtered.length === 0 ? (
					...
					<VStack align="stretch" gap="2">
						{filtered.map((dl: IDownload) => {
							const statusConfig = STATUS_LABELS[dl.status]
								? { color: STATUS_COLORS[dl.status], icon: STATUS_ICONS[dl.status], label: STATUS_LABELS[dl.status] }
