...
import {
	Box, Text, HStack, VStack, Input, Portal, Menu,
} from '@chakra-ui/react';
import {
	ThreadListPrimitive,
} from '@assistant-ui/react';
import {
	MoreHorizontalIcon, TrashIcon, PencilIcon, CheckIcon,
...
function RenamePopover({ value, onSave, onCancel }: { value: string; onSave: (v: string) => void; onCancel: () => void }) {
	...
	return (
		<HStack gap="1" onClick={(e) => e.stopPropagation()}>
			<Input
				ref={ref} size="xs" value={text} onChange={(e) => setText(e.target.value)}
				onKeyDown={(e) => { if (e.key === 'Enter') onSave(text); if (e.key === 'Escape') onCancel(); }}
				bg="var(--wc-bg-card)" borderColor="var(--wc-border-hover)" color="var(--wc-text-primary)"
				fontSize="12px" h="26px" px="2"
			/>
...
function ConfirmDialog({ message, onConfirm, onCancel }: { message: string; onConfirm: () => void; onCancel: () => void }) {
	return (
		<Box position="fixed" top="0" left="0" right="0" bottom="0" bg="var(--wc-overlay-modal)" zIndex={100}
			display="flex" alignItems="center" justifyContent="center" onClick={onCancel}>
			<Box bg="var(--wc-bg-elevated)" borderWidth="1px" borderColor="var(--wc-border-overlay)"
				borderRadius="lg" p="5" maxW="360px" w="90%" onClick={(e) => e.stopPropagation()}>
				<Text fontSize="13px" color="var(--wc-text-primary)" mb="4">{message}</Text>
				<HStack justify="flex-end" gap="2">
					<Box as="button" px="3" py="1.5" borderRadius="md" fontSize="12px"
						bg="var(--wc-bg-card)" color="var(--wc-text-secondary)" _hover={{ bg: 'var(--wc-bg-active)' }}
						onClick={onCancel}>Cancel</Box>
					<Box as="button" px="3" py="1.5" borderRadius="md" fontSize="12px"
						bg="var(--wc-accent-red-alt)" color="var(--wc-special-white)" _hover={{ bg: 'var(--wc-accent-red)' }}
						onClick={onConfirm}>Delete</Box>
				</HStack>
			</Box>
		</Box>
	);
...
// ThreadNode
// ============================================================
function ThreadNode({ node, expanded, setExpanded }: { node: TreeEntry; expanded: boolean; setExpanded: (v: boolean) => void }) {
	...
	const metaFields = useMemo(() => {
		try {
			...
			return { starred: !!m.starred, serverId: m.serverId ?? null, tags: m.tags ?? [], enableAutoEmbed: !!m.enableAutoEmbed };
		} catch {
	...
	const displayText = useMemo(() => {
		...
		return (
			<>
			<Box
				w="100%" className={`group ${selected ? 'selected' : ''}`}
				...
				borderRadius="lg" px="3" py="1" _hover={{ bg: 'var(--wc-bg-hover)' }}
				overflow="hidden"
			>
				{node.children && node.children.length > 0 && (
					<Box
						as="button" onClick={(e) => { e.stopPropagation(); setExpanded(!expanded); }}
						style={{ cursor: 'pointer', display: 'flex', alignItems: 'center' }}
						p="0.5" _hover={{ bg: 'var(--wc-bg-hover)' }} borderRadius="sm"
					>
						{expanded
				...
				{renaming ? (
					<Box flex="1" px="2" py="1">
						<RenamePopover value={thread.title} onSave={(v) => { actions?.onRenameThread(thread.id, v); setRenaming(false); }} onCancel={() => setRenaming(false)} />
					</Box>
				) : (
					<>
						<Box flex="1" display="flex" flexDirection="column" overflow="hidden">
							<HStack>
								...
								<Text fontSize="13px" color="var(--wc-text-primary)" overflow="hidden" textOverflow="ellipsis" whiteSpace="nowrap" minW={0}>
									{thread.title ?? 'New Chat'}
								</Text>
								<Text fontSize="12px" color="var(--wc-text-faint)">{displayText}</Text>
							</HStack>
						</Box>
						<Box position="relative">
							<Menu.Root positioning={{ getAnchorRect }}>
								<Menu.Trigger asChild>
									<Box ref={triggerRef as any} as="button" cursor="pointer" p="1" mr="1" borderRadius="sm"
										opacity={0} className="group-hover:!opacity-50" _hover={{ bg: 'var(--wc-bg-hover)' }}
										type="button" onClick={(e) => e.stopPropagation()}>
										<MoreHorizontalIcon size={13} style={{ color: 'var(--wc-text-muted)' }} />
									</Box>
								</Menu.Trigger>
								<Menu.Positioner>
									<Menu.Content bg="var(--wc-bg-elevated)" borderWidth="1px" borderColor="var(--wc-border-overlay)"
										borderRadius="md" py="1" minW="120px" onClick={(e) => e.stopPropagation()}>
										<Menu.Item value="rename" onClick={() => setRenaming(true)} style={{ fontSize: '12px', color: 'var(--wc-text-primary)' }}>
											<HStack gap="2"><PencilIcon size={12} /><Text>Rename</Text></HStack>
										</Menu.Item>
										<Menu.Item value="star" onClick={() => actions?.onSetStarred(thread.id, !metaFields.starred)} style={{ fontSize: '12px', color: 'var(--wc-text-primary)' }}>
											<HStack gap="2"><IoStarSharp size={12} style={{ color: 'var(--wc-text-primary)' }} /><Text>{metaFields.starred ? 'Unstar' : 'Star'}</Text></HStack>
										</Menu.Item>
										<Menu.Item value="delete" onClick={() => actions?.onDeleteThread(thread.id)} style={{ fontSize: '12px', color: 'var(--wc-accent-red)' }}>
											<HStack gap="2"><TrashIcon size={12} /><Text>Delete</Text></HStack>
										</Menu.Item>
									</Menu.Content>
								</Menu.Positioner>
							</Menu.Root>
					...
					{expanded && node.children && node.children.length > 0 && (
						<Box pl="4" my="1" maxH="600px" overflowY="auto"
							css={{ '&::-webkit-scrollbar': { width: '4px' }, '&::-webkit-scrollbar-thumb': { background: 'var(--wc-text-disabled)', borderRadius: '2px' } }}>
			...
			// FolderNode
			// ============================================================
			function FolderNode({ node, expanded, setExpanded }: { node: TreeEntry; expanded: boolean; setExpanded: (v: boolean) => void }) {
	...
	return (
		<>
			<Box w="full" my="1" borderRadius="lg" border="1px solid var(--wc-border-default)" bg="var(--wc-bg-subtle)" transition="background 0.15s">
			<HStack gap="1" px="2" py="1.5" cursor="pointer" borderRadius="md" position="relative"
				_hover={{ bg: 'var(--wc-bg-card)' }} onClick={handleToggle}>
					{expanded
					...
					{renaming ? (
						<RenamePopover value={folder.name} onSave={(v) => { actions?.onRenameFolder(folder.id, v); setRenaming(false); }} onCancel={() => setRenaming(false)} />
					) : (
								<Text flex="1" fontSize="14px" fontWeight="500" color="var(--wc-text-secondary)" overflow="hidden" textOverflow="ellipsis" whiteSpace="nowrap" ml="1" minW={0}>
							{folder.name}
						</Text>
					)}
					<Text fontSize="12px" color="var(--wc-text-faint)" flexShrink={0}>{threadCount}</Text>
					<Menu.Root positioning={{ getAnchorRect }}>
						<Menu.Trigger asChild>
							<Box ref={triggerRef as any} as="button" opacity={0.4} cursor="pointer" p="0.5"
								className="group-hover:!opacity-100" _hover={{ opacity: 1, bg: 'var(--wc-bg-hover)' }}
								borderRadius="sm" type="button" onClick={(e) => e.stopPropagation()}>
						...
						<Menu.Positioner>
							<Menu.Content bg="var(--wc-bg-elevated)" borderWidth="1px" borderColor="var(--wc-border-overlay)"
								...
								<Menu.Item value="delete" onClick={() => actions?.onDeleteFolder(folder.id)} style={{ fontSize: '12px', color: 'var(--wc-accent-red)' }}>
									<HStack gap="2"><TrashIcon size={12} /><Text>Delete</Text></HStack>
								</Menu.Item>
							</Menu.Content>
						</Menu.Positioner>
...
export const ThreadList = React.memo(({ onOpenSearch }: { onOpenSearch?: () => void }) => {
	...
	const [confirmDelete, setConfirmDelete] = useState<{ type: 'folder' | 'thread'; id: string } | null>(null);
	...
	}), [handleRenameThread, handleDeleteThread, handleSetStarred, handleSelectThread, handleRenameFolder, handleDeleteFolder]);
	...
	const sortLabels = useMemo(() => ({
		updatedAt: 'Updated', createdAt: 'Created', title: 'Name', messageCount: 'Tokens',
	}), []);
			...
			<ThreadListPrimitive.Root className="aui-root aui-thread-list-root flex flex-col flex-1 min-h-0">
				{onOpenSearch && (
					<Box flexShrink={0} mb="2" px="3">
						<Box as="button" w="full" px="3" py="2" borderRadius="md" borderWidth="1px"
							borderColor="var(--wc-border-subtle)" bg="var(--wc-bg-card)" color="var(--wc-text-muted)"
							_hover={{ bg: 'var(--wc-bg-hover)', color: 'var(--wc-text-primary)' }}
							display="flex" alignItems="center" justifyContent="center" gap="2" fontSize="13px"
							cursor="pointer" onClick={onOpenSearch}>
							<SearchIcon size={15} /><Text>Search</Text>
						</Box>
					</Box>
				)}
				...
				<HStack px="3" gap="1" mb="2" justify="space-between" alignItems="center" flexShrink={0}>
					<HStack gap="1">
						<Box as="button" px="2.5" py="1" borderRadius="md" fontSize="12px"
							color="var(--wc-text-muted)" bg="var(--wc-bg-subtle)" _hover={{ bg: 'var(--wc-bg-hover)' }}
							onClick={cycleSortField} title="Click to change sort field">
							{sortLabels[sortField]}
						</Box>
						<Box as="button" p="1" borderRadius="md" color="var(--wc-text-faint)"
					...
					<HStack gap="1">
						<Box as="button" p="1" borderRadius="md" color="var(--wc-text-faint)"
							_hover={{ color: 'var(--wc-text-secondary)' }} onClick={handleCreateFolder}
							title="New folder" mt="1">
				...
				<Box px="3" flex="1" overflowY="auto"
					css={{ '&::-webkit-scrollbar': { width: '4px' }, '&::-webkit-scrollbar-thumb': { background: 'var(--wc-text-disabled)', borderRadius: '2px' } }}
					borderTop="1px solid var(--wc-border-subtle)" pt="2">
