...
export const VoiceInput = React.memo(({ threadId, onTranscript, aui, onStreamChange }: IVoiceInputProps) => {
	...
	const handleVADToggle = useCallback(async () => {
		...
		// Keep at most one live VAD session even if the global flag and the
		// component-local refs briefly drift out of sync.
	...
	// A thread switch resets vadActive in the shared store. Tear down the
	// component-owned microphone session on that external true -> false edge.
