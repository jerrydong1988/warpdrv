						...
						return (
							<Box key={serverName}>
								...
								>
									...
									<Text as="span" fontSize="8px" fontWeight={400} color="var(--wc-text-faint)">
										...
										{state.tools.map(tool => {
											...
											return (
												...
												>
													...
													<Text as="span" overflow="hidden" textOverflow="ellipsis" whiteSpace="nowrap">{tool.name}</Text>
												</Box>
											);
										})}
									</Box>
...
const TodoPanel = React.memo(() => {
	...
	const todos = useStore(s => {
		...
		return (s.getCurrentThreadState()?.todos as ITodoItem[]) || EMPTY_TODOS;
	});
	...
	api.onReady(() => {
		...
		api.registerSlashCommand({
			...
			execute: async (_api, params, extraParams) => {
				await createGuardrailApi({
					...
					triggerOnTools: parseToolValue(String(params.tools ?? '')),
				});
				toggleActiveGuardrail(String(params.name ?? ''), true);
			},
		});
		...
		api.registerSlashCommand({
			...
			execute: async (_api, params) => {
				toggleActiveGuardrail(String(params.name ?? ''), params.action === 'on');
			},
		});
		...
		api.registerSlashCommand({
			...
			execute: async (_api, params, extraParams) => {
				await createModeApi({
					name: String(params.name ?? ''),
					scope: 'global',
					color: String(params.color ?? '#a78bfa'),
					prompt: extraParams?.prompt || undefined,
					allowedTools: parseToolValue(String(params.tools ?? '')),
					activeGuardrails: parseGuardrailValue(String(params.guardrails ?? '')),
				});
			},
		});
